...
export const api = {
  ...
  issueWarning: (id: string, body: { ym?: string; reason?: string; affectsPoints?: boolean; wtype?: string; issuedDate?: string; validUntil?: string | null; salaryDeductionPct?: number }) =>
  ...
  refund: (orderId: string, amountFils: number, opts: { reasonCategory: 'customer_cancellation' | 'quality_issue' | 'missing_item' | 'other'; reason?: string; cancelEvent?: boolean; itemLabel?: string }) =>
    request<any>(`/api/admin/orders/${orderId}/refund`, {
      method: 'POST',
      body: JSON.stringify({ amountFils, reasonCategory: opts.reasonCategory, reason: opts.reason, cancelEvent: !!opts.cancelEvent, itemLabel: opts.itemLabel }),
    }),
  ...
  regenerateCampaign: (id: number) =>
    request<any>(`/api/admin/marketing/campaigns/${id}/regenerate`, { method: 'POST' }),
  ...
  serviceCreate: (body: { name: string; priceFils: number; categoryId: string; celebrationTypes?: string[]; pricingKind?: 'flat' | 'per_piece' | 'per_child'; active?: boolean }) => request(`/api/admin/services`, { method: 'POST', body: JSON.stringify(body) }),
  ...
  finCreateInvoice: (body: Record<string, unknown> & { addressNote?: string | null }) => request<any>('/api/admin/finance/invoices', { method: 'POST', body: JSON.stringify(body) }),
  ...
  finInvoicePayment: (id: number, paidFils: number) => request<any>(`/api/admin/finance/invoices/${id}/payment`, { method: 'PATCH', body: JSON.stringify({ paidFils }) }),
  finInvoiceReminder: (id: number, on: boolean) => request<any>(`/api/admin/finance/invoices/${id}/reminder`, { method: 'PATCH', body: JSON.stringify({ on }) }),
  finUpdateInvoice: (id: number, body: Record<string, unknown> & { addressNote?: string | null }) => request<any>(`/api/admin/finance/invoices/${id}`, { method: 'PATCH', body: JSON.stringify(body) }),
  ...
  finUpdateReceipt: (id: number, body: Record<string, unknown> & { addressNote?: string | null; celebrationType?: string | null }) => request<any>(`/api/admin/finance/receipts/${id}`, { method: 'PATCH', body: JSON.stringify(body) }),
  ...
  finCreateReceipt: (body: Record<string, unknown> & { addressNote?: string | null; celebrationType?: string | null }) => request<any>('/api/admin/finance/receipts', { method: 'POST', body: JSON.stringify(body) }),
