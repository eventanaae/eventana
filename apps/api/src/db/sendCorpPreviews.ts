...
export async function sendCorpPreviewsOnce(): Promise<void> {
  const guard = await pool.query(`SELECT 1 FROM app_kv WHERE k = 'corp_previews_sent_v3'`).catch(() => ({ rowCount: 0 }));
  ...
  await pool.query(`INSERT INTO app_kv (k, v) VALUES ('corp_previews_sent_v3', now()) ON CONFLICT (k) DO UPDATE SET v = now()`).catch(() => {});
