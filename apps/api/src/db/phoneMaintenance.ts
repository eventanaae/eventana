...
export async function phoneMaintenanceFromEnv(): Promise<void> {
  ...
  try {
    ...
    if (mode === 'clean+email' && review.length > 0) {
      ...
      // settings.value is jsonb — store the date as a JSON string.
      const guard = await pool.query(
        `INSERT INTO settings (key, value) VALUES ('phone.marshaEmailedOn', to_jsonb(to_char(now(),'YYYY-MM-DD')))
         ON CONFLICT (key) DO UPDATE SET value = to_jsonb(to_char(now(),'YYYY-MM-DD'))
         WHERE settings.value IS DISTINCT FROM to_jsonb(to_char(now(),'YYYY-MM-DD'))
         RETURNING key`,
      );
