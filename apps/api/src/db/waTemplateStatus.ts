...
export async function waTemplateStatusFromEnv(): Promise<void> {
  ...
  const url = `https://graph.facebook.com/${config.meta.graphVersion}/${waba}/message_templates?fields=name,status,category,language&limit=200`;
  try {
    const res = await fetch(url, { headers: { authorization: `Bearer ${token}` } });
    const json = (await res.json()) as any;
    if (!res.ok) { console.log(`[wa-status] ${res.status}: ${JSON.stringify(json).slice(0, 240)}`); return; }
    const rows = (json.data ?? []) as Array<{ name: string; status: string; language: string; category: string }>;
    console.log(`[wa-status] ${rows.length} template(s):`);
    for (const t of rows.sort((a, b) => a.name.localeCompare(b.name))) {
      console.log(`[wa-status] ${t.name} (${t.language}) = ${t.status} [${t.category}]`);
    }
  } catch (e) {
